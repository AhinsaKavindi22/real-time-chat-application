/**
 * RightSidebar Component
 * Displays user profile details and shared media in the right sidebar
 * Only visible when a user is selected and on desktop view
 */

import React, { useContext, useEffect, useState } from 'react'
import assets, { imagesDummyData } from '../assets/assets'
import { ChatContext } from '../../context/ChatContext';
import { AuthContext } from '../../context/AuthContext';


const RightSidebar = () => {


  const { value1 } = useContext(ChatContext);
  const { selectedUser, messages } = value1;

  const { value } = useContext(AuthContext);
  const { logout, onlineUsers } = value;

  const [msgImages, setMsgImages] = useState([]);

  // get all images from messages and set them to state
  useEffect(() => {
    setMsgImages(
      messages.filter(msg => msg.image).map(msg => msg.image)
    )
  }, [messages])

  // Only render if a user is selected
  return selectedUser && (
    <div className={`bg-[#8185B2]/10 text-white w-full relative overflow-y-scroll 
      ${selectedUser ? "max-md:hidden" : ""}`}>

      {/* User Profile Section */}
      <div className='pt-16 flex flex-col items-center gap-2 text-xs font-light mx-auto'>
        {/* Profile Picture */}
        <img
          src={selectedUser?.profilePic || assets.avatar_icon}
          alt={`${selectedUser.fullName}'s profile`}
          className='w-20 aspect-[1/1] rounded-full'
        />
        {/* User Name with Online Status */}
        <h1 className='px-10 text-xl font-medium mx-auto flex items-center gap-2'>
          {/* Online Status Indicator */}
          {onlineUsers.includes(selectedUser._id) &&
            <p className='w-2 h-2 rounded-full bg-green-500'></p>}
          {selectedUser.fullName}
        </h1>
        {/* User Bio */}
        <p className='px-10 mx-auto'>{selectedUser.bio}</p>
      </div>

      {/* Separator Line */}
      <hr className='border-[#fffffff50] my-4' />

      {/* Shared Media Section */}
      <div className='px-5 text-xs'>
        <p>Media</p>
        {/* Media Grid */}
        <div className='mt-2 max-h-[200px] overflow-y-scroll grid grid-cols-2 gap-4 opacity-80'>
          {/* Map through shared images */}
          {msgImages.map((url, index) => (
            <div
              key={index}
              onClick={() => window.open(url)}
              className='cursor-pointer rounded'
            >
              <img
                src={url}
                alt={`Shared media ${index + 1}`}
                className='h-full rounded-md'
              />
            </div>
          ))}
        </div>
      </div>

      {/* Logout Button */}
      <button onClick={() => logout()} className='absolute bottom-5 left-1/2 transform -translate-x-1/2 
        bg-gradient-to-r from-purple-400 to-violet-600 
        text-white border-none text-sm font-light 
        py-2 px-20 rounded-full cursor-pointer'
      >
        Logout
      </button>
    </div>
  )
}

export default RightSidebar